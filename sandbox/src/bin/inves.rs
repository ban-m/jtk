...
fn main() -> std::io::Result<()> {
    ...
    // let max = 4;
    // let chunk_size = 2;
    // let mut aligner = kiley::trialignment::banded::Aligner::new(max, max, max, chunk_size / 2);
    // let (dist, aln) = aligner.align(b"TG", b"TG", b"TG", 1);
    // assert_eq!(dist, 0, "{:?}", aln);
    let band = 2;
    let seqs: Vec<_> = BufReader::new(std::io::stdin())
        .lines()
        .filter_map(|x| x.ok())
        .map(|x| -> Vec<u8> { x.into() })
        .collect();
    kiley::ternary_consensus_by_chunk(&seqs, band);
