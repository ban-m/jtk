...
fn main() -> std::io::Result<()> {
    ...
    eprintln!("{sim_thr}");
    let errors = haplotyper::estimate_error_rate::estimate_error_rate(&ds, sim_thr);
    let median_of_var = errors.median_of_sqrt_err;
    eprintln!("SD\t{median_of_var}");
    if let Ok(mut wtr) = std::fs::File::create("dump.log").map(BufWriter::new) {
        for read in ds.encoded_reads.iter() {
            let error = errors.read(read.id);
            let len = read.original_length;
            writeln!(wtr, "READ\t{}\t{}\t{}", read.id, error, len)?;
        }
        for unit in ds.selected_chunks.iter() {
            for (cl, error) in errors.unit_error_rate[unit.id as usize].iter().enumerate() {
    ...
    for (id, unit, cluster, error) in error_rates {
        let expected = errors.read(id) + errors.unit((unit, cluster));
        println!("{id}\t{unit}\t{cluster}\t{error}\t{expected}");
    }
    Ok(())
}
